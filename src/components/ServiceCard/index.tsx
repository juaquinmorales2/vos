...
const Index: FC<Props> = ({ card: { title, services, description, number, classes } }) => {
  ...
  return (
    <div ref={ref} key={number} className="px-[6vw] md:px-[3vw] pb-[9.5vw]  last:pb-[13vw]">
      <h4 className="text-[3.7vw] md:text-[4.2vw] font-light ">{title}</h4>
      <div className={`flex items-start space-x-[3vw] pt-[3vw] first:border-none ${classes}`}>
        <div className="flex-1">
          <div className="flex flex-wrap space-y-[2vw]">
            {services.map((service: string[], i: number) => {
              return (
                <ul key={i} className="flex items-center space-x-[5vw] text-[1.7vw] md:text-[2vw] font-semibold">
                  {service.map((s) => (
        ...
        <div className="relative flex-1 ">
          <p className="relative z-[2000] line-clamp-4 text-[1.5vw] md:text-[1.8vw] font-medium leading-[170%]">{description}</p>
